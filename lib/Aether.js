...
class Aether {
	...
	async init() {
		this.ws.init();
		this.log.info('[Websocket] initalized.');
		// this.log.info('[Discord] Requesting recommended shard amount ...');
		...
		this.total = Number(process.env.K8S_PODS);
		this.log.info(`Initialized Aether with ${this.total} recommended shards!`);
	}

	launch() {
		this.ws.queue.sort((a, b) => a.id - b.id);
		this.log.info(`Shards in queue: [ ${this.ws.queue.join(', ')} ]`);
		const shard = this.ws.queue.shift();
		shard.socket.send(MessageUtil.encode(new Message(LAUNCH_CLIENT, { shardCount: this.total })));
	}
