...
class Websocket extends Server {
	...
	async init() {
		this.handler.init();
		this.client.log.info('[Websocket] Events loaded, now initializing.');
		this.on('connection', socket => {
			this.client.log.info('<<< [Websocket] Received connection.');
			socket.identified = false;

			socket.on('message', message => {
				this.client.log.info('<<< [Websocket] Received message.');
				this.handler.handle(socket, message);
			});

			socket.on('close', (code, reason) => {
				this.client.log.warn(`<<< [Websocket] Received disconnect (${code}): ${reason}`);
